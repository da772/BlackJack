...
public class Deck {
	...
	private Stack<Integer> usedDeck;
	...
	public Deck(int decksCount) {
		...
		this.usedDeck = new Stack<Integer>();
	...
	public void printDeckInString() {
		//Print every card in the deck in string. 
		int deckCount = this.deck.size();
		for(int i = 0; i < deckCount; i++) {
	...
	public void printUsedPile() {
		int usedDeckCount = this.usedDeck.size();
		for(int i = 0; i < usedDeckCount; i++) {
			System.out.print(convertCardToString(this.usedDeck.get(i)) + " ");
		}
	}
	...
	public void reshuffleUsedCards() {
		//Add used cards back into the deck
		int usedDeckCount = this.usedDeck.size();
		for(int i = 0; i < usedDeckCount; i++) {
			this.deck.add(this.usedDeck.pop());
		}
	}
	...
	public static void main(String[] args) {
		...
		gameDeck.printUsedPile();
		System.out.println("\nPrint current deck: ");
		gameDeck.printDeck();
		System.out.println("\nNow readding used cards to deck..");
		gameDeck.reshuffleUsedCards();
